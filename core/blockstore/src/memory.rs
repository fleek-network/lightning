...
//     ) -> Option<Self::SharedPointer<ContentChunk>> { match bincode::deserialize::<BlockContent>(
//       self.fetch(&Key::chunk_key(*block_hash, block_counter)) .await? .as_slice(), )
//       .expect("Stored content to be serialized properly") { BlockContent::Chunk(content) =>
//       Some(Arc::new(ContentChunk { compression: CompressionAlgorithm::Uncompressed, content, })),
//       _ => None, }
//     }
//
