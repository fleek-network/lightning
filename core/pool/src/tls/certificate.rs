...
pub fn generate(
    identity_secret_key: &NodeSecretKey,
...
pub fn parse_unverified(der_input: &[u8]) -> Result<P2pCertificate, webpki::Error> {
    ...
    for ext in x509.extensions() {
        ...
        if oid == &p2p_ext_oid {
            ...
            let public_key_raw = <[u8; 32]>::try_from(public_key_raw).unwrap();
            let public_key = NodePublicKey(public_key_raw);

            // let signature = Signature::from_slice(signed_key.signature.as_bytes())
            //     .map_err(|_| webpki::Error::UnknownIssuer)?;
            let signature_raw = <[u8; 64]>::try_from(signed_key.signature.as_bytes()).unwrap();
            let signature = NodeSignature(signature_raw);
...
fn make_libp2p_extension(
    ...
    let signature = {
        ...
        let msg = fleek_blake3::hash(&msg);
        identity_secret_key.sign(msg.as_bytes())
    };

    let public_key = identity_secret_key.to_pk();

    let key = SignedKey {
        public_key: OctetStringRef::new(&public_key.0).unwrap(),
        signature: OctetStringRef::new(&signature.0).unwrap(),
    };

    let mut extension_content = Vec::new();
    key.encode_to_vec(&mut extension_content)
        .expect("ASN.1 encoding to succeed");
...
impl P2pCertificate<'_> {
    ...
    pub fn peer_pk(&self) -> NodePublicKey {
        self.extension.public_key
    }

    /// Verify the `signature` of the `message` signed by the secret key corresponding to the public
...
mod tests {
    ...
    fn test_generate_parse() {
        ...
        assert_eq!(secret_key.to_pk(), parsed_cert.extension.public_key);
    }
}
