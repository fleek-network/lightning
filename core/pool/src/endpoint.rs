...
//
//                 tracing::debug!(
//                     "received broadcast send request for peers not in the overlay:
// {not_connected:?}"                 );
//                 tracing::debug!(
...
//                         if ongoing_conn_tx.send(request).await.is_err() {
//                             tracing::error!(
//                                 "failed to send broadcast request to connection task for node
// with index: {index:?}"                             );
//                         }
//                     });
//                 }
...
//             if self.pool.contains_key(&peer_index) &&
// self.redundant_pool.contains_key(&peer_index)             {
...
//                         tracing::info!(
//                             "replacing connection with node with index {peer_index} in redundant
// pool"                         );
//                     }
//                 },
//                 Entry::Vacant(vacant) => {
...
//                             if let Err(e) = self.handle_broadcast_task(broadcast_task) {
//                                 tracing::error!("failed to handle broadcast task after an update:
// {e:?}");                             }
//                         }
...
//                     match peer {
//                         Ok((index, id)) => {
//                             tracing::trace!("task for connection={id:?} with node={index:?}
// ended");                             self.garbage_collect_closed_connections(index, id);
//                         }
//                         Err(e) => {
