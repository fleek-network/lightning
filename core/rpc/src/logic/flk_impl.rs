...
impl<C: Collection> FleekApiServer for FleekApi<C> {
    ...
    async fn get_total_supply(&self) -> RpcResult<HpUfixed<18>> {
        let total_supply = match self.data.query_runner.get_metadata(&Metadata::TotalSupply) {
    ...
    async fn handle_subscription(&self, pending: PendingSubscriptionSink, event_type: EventType) -> SubscriptionResult {
