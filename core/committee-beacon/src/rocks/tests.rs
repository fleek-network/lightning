...
fn test_set_and_get_beacon() {
    ...
    db.set_beacon(epoch, round, commit, reveal);
...
fn test_set_and_get_commits() {
    ...
    db.set_beacon(epoch, round, commit, reveal);
    assert_eq!(query.get_beacon(epoch, commit), Some(reveal));
    assert_eq!(query.get_commit(epoch, round), Some(commit));
...
fn test_get_beacons() {
    ...
    db.set_beacon(epoch, round, commit1, reveal1);
    db.set_beacon(epoch, round, commit2, reveal2);
...
fn test_clear_beacons() {
    ...
    let (commit1, reveal1) = generate_random_beacon();
    let (commit2, reveal2) = generate_random_beacon();
    db.set_beacon(epoch + 1, round, commit1, reveal1);
    db.set_beacon(epoch + 1, round, commit2, reveal2);
