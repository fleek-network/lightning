...
impl InMemoryFs {
    pub fn setup_text_files(&self, files: Vec<(String, String)>) {
        for (path, text) in files {
            ...
            self.mkdir_sync(path.parent().unwrap(), true, Some(0))
                .unwrap();
...
impl FileSystem for InMemoryFs {
    ...
    fn mkdir_sync(&self, path: &Path, recursive: bool, _mode: Option<u32>) -> FsResult<()> {
        let path = normalize_path(path);
        ...
        if let Some(parent) = path.parent() {
            ...
            match entry {
                ...
                None => {
                    if recursive {
                        self.mkdir_sync(parent, true, Some(0))?;
                    } else {
    ...
    async fn mkdir_async(&self, path: PathBuf, recursive: bool, mode: Option<u32>) -> FsResult<()> {
    ...
    fn read_file_sync(
        ...
        match entry {
            Some(entry) => match &*entry {
                PathEntry::File(data) => Ok(data.clone().into()),
