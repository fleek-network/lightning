...
use opentelemetry::metrics::{AsyncInstrumentBuilder, InstrumentBuilder, MeterProvider as _};
...
use opentelemetry_sdk::logs::{BatchLogProcessor, LogProcessor, LogRecord};
use opentelemetry_sdk::metrics::exporter::PushMetricExporter;
use opentelemetry_sdk::metrics::reader::MetricReader;
use opentelemetry_sdk::metrics::{ManualReader, MetricResult, SdkMeterProvider, Temporality};
...
static OTEL_SHARED_RUNTIME_SPAWN_TASK_TX: Lazy<UnboundedSender<BoxFuture<'static, ()>>> =
    Lazy::new(otel_create_shared_runtime);

static OTEL_PRE_COLLECT_CALLBACKS: Lazy<Mutex<Vec<oneshot::Sender<oneshot::Sender<()>>>>> =
    Lazy::new(Default::default);

fn otel_create_shared_runtime() -> UnboundedSender<BoxFuture<'static, ()>> {
    let (spawn_task_tx, mut spawn_task_rx) = mpsc::unbounded::<BoxFuture<'static, ()>>();
...
impl<T: Send> From<tokio::sync::mpsc::Sender<T>> for BatchMessageChannelSender<T> {
    fn from(sender: tokio::sync::mpsc::Sender<T>) -> Self {
        Self { sender }
    }
}

impl<T: Send> opentelemetry_sdk::runtime::TrySend for BatchMessageChannelSender<T> {
    type Message = T;
...
impl<T> From<tokio::sync::mpsc::Receiver<T>> for BatchMessageChannelReceiver<T> {
    ...
    fn new(exporter: opentelemetry_otlp::MetricExporter) -> Self {
        ...
        let worker = async move {
            ...
            let collect_and_export = |collect_observed: bool| {
                ...
                async move {
                    let mut resource_metrics = opentelemetry_sdk::metrics::data::ResourceMetrics {
...
pub fn init(
    ...
    let builtin_instrumentation_scope = opentelemetry::InstrumentationScope::builder("deno")
        .with_version(rt_config.runtime_version.clone())
        .build();

    let id_generator = if otel_config.deterministic {
    ...
    fn new_trace_id(&self) -> TraceId {
        match self {
            ...
            Self::Deterministic { next_trace_id, .. } => {
                let id = next_trace_id.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                let bytes = id.to_be_bytes();
                let bytes = [
                    0, 0, 0, 0, 0, 0, 0, 0, bytes[0], bytes[1], bytes[2], bytes[3], bytes[4],
...
fn parse_trace_id(scope: &mut v8::HandleScope<'_>, trace_id: v8::Local<'_, v8::Value>) -> TraceId {
    if let Ok(string) = trace_id.try_cast() {
        ...
        match value_view.data() {
            v8::ValueViewData::OneByte(bytes) => {
                TraceId::from_hex(&String::from_utf8_lossy(bytes)).unwrap_or(TraceId::INVALID)
            },
...
fn parse_span_id(scope: &mut v8::HandleScope<'_>, span_id: v8::Local<'_, v8::Value>) -> SpanId {
    if let Ok(string) = span_id.try_cast() {
        let value_view = v8::ValueView::new(scope, string);
        match value_view.data() {
            v8::ValueViewData::OneByte(bytes) => {
                SpanId::from_hex(&String::from_utf8_lossy(bytes)).unwrap_or(SpanId::INVALID)
            },
            _ => SpanId::INVALID,
        }
    ...
    ($scope:ident, $name:expr, $value:expr) => {{
        let name = if let Ok(name) = $name.try_cast() {
            ...
            match view.data() {
                ...
                v8::ValueViewData::TwoByte(bytes) => Some(String::from_utf16_lossy(bytes)),
            }
        } else {
            None
        };
...
macro_rules! attr {
  (
    $scope:ident,
    $attributes:expr $(
        => $dropped_attributes_count:expr)?, $name:expr, $value:expr
  ) => {
...
fn op_otel_log<'s>(
    ...
    if let Some(span) = deno_core::_ops::try_unwrap_cppgc_object::<OtelSpan>(scope, span) {
...
fn op_otel_log_foreign(
    ...
    if trace_id != TraceId::INVALID && span_id != SpanId::INVALID {
        log_record.set_trace_context(trace_id, span_id, Some(TraceFlags::new(trace_flags)));
    }

    log_processor.emit(&mut log_record, builtin_instrumentation_scope);
}

fn owned_string<'s>(scope: &mut v8::HandleScope<'s>, string: v8::Local<'s, v8::String>) -> String {
    let x = v8::ValueView::new(scope, string);
    match x.data() {
        v8::ValueViewData::OneByte(bytes) => String::from_utf8_lossy(bytes).into_owned(),
        v8::ValueViewData::TwoByte(bytes) => String::from_utf16_lossy(bytes),
    }
}
...
impl OtelSpan {
    ...
    #[fast]
    fn end(&self, op_state: &mut OpState, end_time: f64) {
...
fn op_otel_span_attribute1<'s>(
    ...
    let Some(span) = deno_core::_ops::try_unwrap_cppgc_object::<OtelSpan>(scope, span) else {
        return;
    };
...
fn op_otel_span_add_link<'s>(
    ...
    if let OtelSpanState::Recording(span) = &mut **state {
        span.links
            .links
            .push(Link::new(span_context, vec![], dropped_attributes_count));
    }
    true
}
...
#[op2(fast)]
fn op_otel_metric_record0(state: &mut OpState, #[cppgc] instrument: &Instrument, value: f64) {
...
fn op_otel_metric_record1(
    ...
    let Some(instrument) =
        deno_core::_ops::try_unwrap_cppgc_object::<Instrument>(&mut *scope, instrument)
    else {
        return;
    };
...
fn op_otel_metric_attribute3<'s>(
    ...
    if let Some(values) = &mut values {
        values
            .attributes
            .reserve_exact((capacity as usize).saturating_sub(values.attributes.capacity()));
        if let Some(kv1) = attr1 {
            values.attributes.push(kv1);
        }
        if let Some(kv2) = attr2 {
