...
const ENCLAVE_URL: &str =
    "https://bafybeiez7pmrjtivpchbyhx7gw7iov76jbkwcgioiawzdgyrpygqr2cgp4.ipfs.flk-ipfs.xyz";
...
fn main() {
    ...
    if let Ok(path) = std::env::var("FN_ENCLAVE_SOURCE") {
        if !path.is_empty() {
            ...
            // cargo output path
            let bin =
                path.join("target/x86_64-fortanix-unknown-sgx/release/fleek-service-sgx-enclave");
