...
fn mint<B: StorageBackend, S: SerdeBackend>(
    ctx: &mut TableRef<UserId, u128, B, S>,
    user: UserId,
    amount: u128,
) -> u128 {
    let balance = ctx.get(user).unwrap_or(0);
    ctx.insert(user, balance + amount);
    balance + amount
}

fn balance<B: StorageBackend, S: SerdeBackend>(
    ctx: &TableRef<UserId, u128, B, S>,
    user: UserId,
) -> u128 {
    ctx.get(user).unwrap_or(0)
}

fn main() {
    let mut u = AtomoBuilder::<InMemoryStorage, DefaultSerdeBackend>::default()
        .with_table::<UserId, u128>("ledger")
        .build()
        .unwrap();
    let q = u.query();
    let table = u.resolve::<UserId, u128>("ledger");
    ...
    println!(
        "To configure use `-t [num threads] -q [num query per thread] -u [num tx per
update]"
    );
    ...
    for _ in 0..num_query_threads {
        ...
        let handle = std::thread::spawn(move || {
            ...
            for _ in 0..num_query_per_thread {
                ...
                q.run(|selector| {
                    let table = table.get(selector);
                    black_box(balance(&table, rng as u8))
                });
            }
    ...
    let updates = if num_query_threads == 0 {
        for _ in 0..num_query_per_thread {
            let r = rng;
            rng = u.run(move |selector| {
                let mut table = table.get(selector);
                let mut rng = r;
                for _ in 0..num_mint_per_update {
                    rng = rng.wrapping_mul(48271) % 0x7fffffff;
                    black_box(mint(&mut table, rng as u8, 17));
                }
                rng
            });
        }
