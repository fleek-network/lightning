...
/// Returns true if the argument's type has a leading `::` in front of it.
pub fn has_leading_colon(arg: &syn::FnArg) -> bool {
    matches!(
        arg,
        syn::FnArg::Typed(syn::PatType {
            ty: box syn::Type::Path(syn::TypePath {
                path: syn::Path {
...
// Valid input:
pub fn resolve_type_path(base: syn::Ident, tp: syn::TypePath) -> Result<syn::TypePath> {
    // todo
    Ok(tp)
}
        ...
        ($($tt:tt)*) => {
            {
                ...
                let parsed: syn::FnArg = syn::parse2(tokens).unwrap();
                println!("---CODE----\n{}\n-----------\n{:#?}", string, parsed);
            }
        }
    }
