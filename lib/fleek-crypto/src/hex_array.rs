//! A custom serde serialize deserialize that uses hex encoding when dealing with a
...
//! struct Data(#[serde(with = "hex_array") [u8; 128])
//! ```
    ...
    S: Serializer,
{
    if serializer.is_human_readable() {
        let t = hex::encode(data);
        serializer.serialize_str(&format!("0x{}", t))
    } else {
    ...
    D: Deserializer<'de>,
{
    if deserializer.is_human_readable() {
        ...
        let str = if let Some(tmp) = string.strip_prefix("0x") {
            tmp
        } else {
            &string
        };

        let vec = hex::decode(str).map_err(serde::de::Error::custom)?;
