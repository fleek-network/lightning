...
/**/      0x0b // expression end
  ...
  if (isWasmSimdAvailable) {
    const output = roundVariableNameLot.map(n => `(local $${n} v128)`);
    const push = output.push.bind(output);

    const wasm = new Uint8Array(8 << 10);
    let currentOffset = 0;
    const put = (value) => { wasm.put(value, currentOffset); currentOffset += value.length; };
    ...
    put([
      ...
      // T0: func compress4(i32, i32, i32, i32) -> ()
    ...
    // (local.set $s8 (i32x4.splat (i32.const 0x6A09E667)))
    // (local.set $s9 (i32x4.splat (i32.const 0xBB67AE85)))
    // (local.set $s10 (i32x4.splat (i32.const 0x3C6EF372)))
    // (local.set $s11 (i32x4.splat (i32.const 0xA54FF53A)))
    // ;; s[14] = BLOCK_LEN
    // (local.set $s14 (i32x4.splat (i32.const 64)))
    ...
    const gen = {
      store(i) {
        push(`(local.set $${roundVariableNameLot[i]})`);
      },
      load(i) {
        push(`(local.get $${roundVariableNameLot[i]})`);
      },
      add() {
        push(`(i32x4.add)`);
      },
      l2xsrs(a, b, bits) {
        ...
        push(
          '(v128.xor)',
          `(local.tee $${roundVariableNameLot[a]})`,
          `(i32.const ${bits})`,
          '(i32x4.shr_u)'
        );
        gen.load(a);
        push(`(i32.const ${32 - bits})`, '(i32x4.shl)', '(v128.or)');
        gen.store(a);
      },
    };
