...
fn bench_generic_verify_proof_rocksdb_jmt_keccak256_simple(b: &mut Bencher) {
    ...
    generic_bench_verify_proof::<_, JmtStateTree<_, DefaultSerdeBackend, KeccakHasher>>(
        b,
...
fn bench_generic_verify_proof_rocksdb_jmt_blake3_simple(b: &mut Bencher) {
    let temp_dir = tempdir().unwrap();
    generic_bench_verify_proof::<_, JmtStateTree<_, DefaultSerdeBackend, Blake3Hasher>>(
        b,
        rocksdb_builder(&temp_dir),
        DATA_COUNT_SIMPLE,
    );
...
fn bench_generic_verify_proof_rocksdb_jmt_sha256_simple(b: &mut Bencher) {
    let temp_dir = tempdir().unwrap();
    generic_bench_verify_proof::<_, JmtStateTree<_, DefaultSerdeBackend, Sha256Hasher>>(
        b,
        rocksdb_builder(&temp_dir),
        DATA_COUNT_SIMPLE,
    );
...
fn bench_generic_verify_proof_rocksdb_mpt_keccak256_simple(b: &mut Bencher) {
    let temp_dir = tempdir().unwrap();
    generic_bench_verify_proof::<_, MptStateTree<_, DefaultSerdeBackend, KeccakHasher>>(
        b,
        rocksdb_builder(&temp_dir),
        DATA_COUNT_SIMPLE,
    );
...
fn bench_generic_verify_proof_rocksdb_mpt_blake3_simple(b: &mut Bencher) {
    let temp_dir = tempdir().unwrap();
    generic_bench_verify_proof::<_, MptStateTree<_, DefaultSerdeBackend, Blake3Hasher>>(
        b,
        rocksdb_builder(&temp_dir),
        DATA_COUNT_SIMPLE,
    );
...
fn bench_generic_verify_proof_rocksdb_mpt_sha256_simple(b: &mut Bencher) {
    let temp_dir = tempdir().unwrap();
    generic_bench_verify_proof::<_, MptStateTree<_, DefaultSerdeBackend, Sha256Hasher>>(
        b,
        rocksdb_builder(&temp_dir),
        DATA_COUNT_SIMPLE,
    );
...
fn generic_bench_verify_proof<C: StorageBackendConstructor, T>(
    b: &mut Bencher,
    builder: C,
    data_count: usize,
) where
    T: StateTree<Storage = C::Storage>,
{
    ...
    db.query().run(|ctx| {
        state_root = Some(T::get_state_root(ctx).unwrap());
